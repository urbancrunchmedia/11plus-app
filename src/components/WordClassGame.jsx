...
function prepareQuestions(level, count, practice, avoidKeys) {
  if (practice) return getMisses(SKILL).map((b) => ({ segments: b.segments, answer: b.answer, askFor: b.askFor, why: b.why }));
  const pick = (obj) => (level === "all"
    ? [...(obj.A || []), ...(obj.B || []), ...(obj.C || [])]
    : [...(obj[level] || [])]);
  const spot = pick(wordClassSpot).map((b) => ({ segments: b.segments, answer: b.answer, askFor: b.askFor, why: b.why }));
...
export default function WordClassGame({ level, totalQuestions = 20, onHome, muted: mutedProp, practice = false }) {
  ...
  function handleAnswer(idx) {
    ...
    if (correct) {
      ...
      if (!muted) playWrong();
      addMiss(SKILL, id, { segments: q.segments, answer: q.answer, askFor: q.askFor, why: q.why });
      setStreak(0);
      setTotalWrong((w) => w + 1);
    }
  ...
  if (gameComplete) {
    return (
      <GameComplete results={results} totalWrong={totalWrong} timeTaken={elapsed}
        onPlayAgain={handlePlayAgain} onPracticeMisses={handlePracticeMisses} onHome={onHome} level={level} gameType="wordClass" totalQuestions={total} />
    );
  }
